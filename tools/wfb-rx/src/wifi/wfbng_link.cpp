...
void WfbngLink::handle_80211_frame(const Packet &packet) {
    ...
    // ArduDeck: forward destination is configurable (--host) so a PC receiver
    // can send video to ArduDeck on another machine. Defaults to localhost.
    std::string client_addr = GuiInterface::Instance().forwardHost;
...
void WfbngLink::handle_rtp(uint8_t *payload, uint16_t packet_size) {
    ...
    // ArduDeck: honor the configurable forward host (--host) on Windows too.
    serverAddr.sin_addr.s_addr = inet_addr(GuiInterface::Instance().forwardHost.c_str());
